        ...
        >
          ${this.entityValues.map((t) => {
            ...
            if (t.action && t.action.service) {
              ...
              if ("fire-dom-event" === i)
                e = () => {
                  this.fire("ll-custom", { entity_id: t.entity, ...s });
                };
              else {
                const [n, r] = i.split(".");
                e = () => {
                  this._hass.callService(n, r, { entity_id: t.entity, ...s });
                };
              }
            }
            const i = { ...t, onClick: e };
            if (!t.attribute) {
              if (t.type && t.type.startsWith("custom:")) {
                const e = t.type.split(":")[1];
                let s = "";
                return (
                  "calendar-card" === e && (s = "small-text"),
                  this.renderCustomElement(e, i, s)
                );
              }
              switch (t.domain) {
                case "light":
                case "switch":
                case "input_boolean":
                  return this.renderAsToggle(i);
                case "cover":
                  return this.renderDomainCover(i);
                case "media_player":
                  return this.renderDomainMediaPlayer(i);
              }
            }
